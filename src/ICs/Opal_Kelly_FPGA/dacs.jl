...
function setDacManual(rhd::FPGA,value)
    if (OPEN_EPHYS)
        SetWireInValue(rhd,OPEN_EPHYS_WireInManualTriggers,value<<15,0xffff0000)
    else
        SetWireInValue(rhd,WireInManualTriggers,value<<15,0xffff0000)
    end
    UpdateWireIns(rhd)
end
