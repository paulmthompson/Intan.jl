...
#GtkRadioMenuItem(group::Ptr{Nothing} = C_NULL) = GtkRadioMenuItem(ccall((:gtk_radio_menu_item_new, Gtk.libgtk), Ptr{GObject}, (Ptr{Nothing},),group))

#GtkRadioMenuItem(label::String) = GtkRadioMenuItem(ccall((:gtk_radio_menu_item_new_with_mnemonic,Gtk.libgtk),Ptr{GObject},(Ptr{Nothing},Ptr{UInt8}),C_NULL,string(label)))
...
#set_active!(group::GtkRadioMenuItem) = ccall((:gtk_check_menu_item_set_active,Gtk.libgtk),Nothing,(Ptr{GObject},Bool),group,true)
