    ...
    if han.c_right_top != event_id
        ...
        if event_id==3
            if han.c_right_bottom != 5
                set_gtk_property!(han.rb2[5],:active,true)
            end
        elseif event_id==4
            if han.c_right_bottom != 6
                set_gtk_property!(han.rb2[6],:active,true)
            end
        else
            if (han.c_right_bottom == 5)|(han.c_right_bottom == 6)
                set_gtk_property!(han.rb2[8],:active,true)
            end
        end
    end
    ...
    if han.c_right_bottom != event_id
        ...
        if event_id==5
            if han.c_right_top!=3
                set_gtk_property!(han.rb1[3],:active,true)
            end
        elseif event_id==6
            if han.c_right_top != 4
                set_gtk_property!(han.rb1[4],:active,true)
            end
        elseif event_id == 7
            ctx=Gtk.getgc(han.c)
            ...
            if (han.c_right_top == 3)|(han.c_right_top == 4)
                set_gtk_property!(han.rb1[1],:active,true)
            end
        end
    end
...
function set_slider(han::Gui_Handles,chan_num::Int64)
    ...
    if han.num16*chan_num>length(han.enabled)
        set_gtk_property!(han.adj2,:upper,han.num16*chan_num-length(han.enabled))
        set_gtk_property!(han.adj,:upper,div(length(han.enabled),chan_num)+1)
    else
        set_gtk_property!(han.adj2,:upper,chan_num)
        set_gtk_property!(han.adj,:upper,div(length(han.enabled),chan_num))
    end
    set_gtk_property!(han.adj2, :value, han.num)
    set_gtk_property!(han.adj, :value, han.num16)
    nothing
end
